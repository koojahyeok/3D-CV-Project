...
class LearnFocal(nn.Module):
    def __init__(self, H, W, n_images, order=2, init_focal=None, init_weight=2, init_global_bias=-300, init_local_bias=None, req_grad=True, fx_only=True):
    ...
    def get_all_focals(self):
        ...
        out = fxfy * self.weight_global + self.bias_global + self.bias_local ** self.order
    ...
    def forward(self, idx):  # the i=None is just to enable multi-gpu training
        ...
        fxfy = fxfy.to(self.weight_global.device)
        out = fxfy * self.weight_global + self.bias_global + self.bias_local[idx] ** self.order
        
        return torch.clamp(out, min=0)  # focal length should be positive


if __name__ == "__main__":
    model = LearnFocal(100, 200, 5, init_focal=torch.FloatTensor([200, 400, 300, 400, 500]))
