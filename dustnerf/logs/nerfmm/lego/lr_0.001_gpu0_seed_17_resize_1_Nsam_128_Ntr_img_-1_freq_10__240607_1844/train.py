        ...
        if args.model == 'nerfmm':
            if epoch_i % args.eval_cam_interval == 0 and epoch_i > 0:
                with torch.no_grad():
                    ...
                    #                                                                         scene_train.focal.mean()))
                    print(fxfy.shape, scene_train.focal.shape)
                    if torch.is_tensor(fxfy):
                        L1_focal = torch.abs(fxfy - scene_train.focal.unsqueeze(0)).mean().item()
                    else:
                        L1_focal = np.abs(fxfy - scene_train.focal.unsqueeze(0)).mean()
