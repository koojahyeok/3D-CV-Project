...
def main(args):
    ...
    # imageio.mimwrite(os.path.join(video_out_dir, 'img.mp4'), imgs, fps=30)                  # quality 제거
    # imageio.mimwrite(os.path.join(video_out_dir, 'depth.mp4'), depths, fps=30)              # quality 제거
    ...
    for i in range(c2ws.shape[0]):
        cv2.imwrite(os.path.join(img_out_dir, str(i).zfill(4) + '.png'), imgs[i])
        cv2.imwrite(os.path.join(depth_out_dir, str(i).zfill(4) + '.png'), depths[i])

    # Video output using cv2
    fourcc = cv2.VideoWriter_fourcc(*'mp4v')
    img_video = cv2.VideoWriter(os.path.join(video_out_dir, 'img.mp4'), fourcc, 30, (scene_train.W, scene_train.H))
    depth_video = cv2.VideoWriter(os.path.join(video_out_dir, 'depth.mp4'), fourcc, 30, (scene_train.W, scene_train.H))

    for i in range(c2ws.shape[0]):
        img_video.write(imgs[i])
        depth_video.write(depths[i])

    img_video.release()
    depth_video.release()

    # GIF output using matplotlib
    for i in range(c2ws.shape[0]):
        plt.imsave(os.path.join(video_out_dir, str(i).zfill(4) + '.png'), imgs[i])
        plt.imsave(os.path.join(video_out_dir, str(i).zfill(4) + '.png'), depths[i])

    # Convert images to GIF
    img_files = sorted(glob.glob(os.path.join(video_out_dir, 'img', '*.png')))
    depth_files = sorted(glob.glob(os.path.join(video_out_dir, 'depth', '*.png')))

    img_gif = os.path.join(video_out_dir, 'img.gif')
    depth_gif = os.path.join(video_out_dir, 'depth.gif')

    with imageio.get_writer(img_gif, mode='I', duration=0.1) as writer:
        for file in img_files:
            image = imageio.imread(file)
            writer.append_data(image)

    with imageio.get_writer(depth_gif, mode='I', duration=0.1) as writer:
