...
def main(args):
    ...
    # o3d.io.write_triangle_mesh(os.path.join(args.ckpt_dir, 'draw.ply'), geometry_to_draw)
    # o3d.io.write_triangle_mesh(os.path.join(args.ckpt_dir, 'frustum_colmap.ply'), frustum_colmap_list)
    # o3d.io.write_line_set(os.path.join(args.ckpt_dir, 'line_set.ply'), line_set)
    for i, geom in enumerate(geometry_to_draw):
        if isinstance(geom, o3d.geometry.TriangleMesh):
            o3d.io.write_triangle_mesh(os.path.join(args.ckpt_dir, f'geometry_{i}.ply'), geom)
        elif isinstance(geom, o3d.geometry.PointCloud):
            o3d.io.write_point_cloud(os.path.join(args.ckpt_dir, f'geometry_{i}.ply'), geom)
        elif isinstance(geom, o3d.geometry.LineSet):
            o3d.io.write_line_set(os.path.join(args.ckpt_dir, f'geometry_{i}.ply'), geom)
        else:
